...
def get_utr(file, genelist):
    genelist1 = genelist.index
    df = pd.read_csv(file, sep="\t")
    df_list = df[df["gene"]==genelist1]
    df_list_s = df_list[["gene", "annot1"]]
    pass
...
def main():
    ...
    # A7_table.to_csv("WTvsEZH2sh/A7grouped.xls", sep="\t", header=False)
    # A8_table.to_csv("WTvsEZH2sh/A8grouped.xls", sep="\t", header=False)
    # A6_table.to_csv("WTvsEZH2sh/A6grouped.xls", sep="\t", header=False)

    # print A7uniq_df["annot1"].unique()
    ...
    A6_final.to_csv("WTvsEZH2sh/CtrvsA7A8_final.xls", sep="\t")
